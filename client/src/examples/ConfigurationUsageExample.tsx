    ...
    <div className="rounded-lg bg-white p-6 shadow-md">
      ...
      <div className="mb-4 space-y-2 rounded-md bg-gray-50 p-4">
        ...
        <ul className="ml-6 list-disc space-y-1 text-sm text-gray-600">
          <li>
            Device Refresh Interval: <span className="font-mono">{deviceRefreshInterval}ms</span>
          </li>
          <li>
            Server Poll Interval: <span className="font-mono">{serverPollInterval}ms</span>
          </li>
          <li>
            Real-time Updates:{' '}
            <span className={realTimeUpdatesEnabled ? 'text-green-600' : 'text-red-600'}>
              {realTimeUpdatesEnabled ? 'Enabled' : 'Disabled'}
            </span>
          </li>
          <li>
            Device Polling:{' '}
            <span className={devicePollingEnabled ? 'text-green-600' : 'text-red-600'}>
              {devicePollingEnabled ? 'Enabled' : 'Disabled'}
            </span>
          </li>
          <li>
            Debug Mode:{' '}
            <span className={debugModeEnabled ? 'text-yellow-600' : 'text-gray-600'}>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="pt-2">
          <button
            className={`rounded ${isConfigurationDirty ? 'bg-purple-600 hover:bg-purple-700' : 'cursor-not-allowed bg-gray-300'} px-4 py-2 text-white`}
          ...
          {isConfigurationDirty && (
            <p className="mt-1 text-xs text-amber-600">You have unsaved configuration changes</p>
          )}
        </div>
      </div>
      ...
      {!isPolling && (
        <div className="mt-4 rounded-md bg-yellow-50 p-3 text-sm text-yellow-700">
          Note: Automatic data updates are not active. Enable both Device Polling and Real-Time
          Updates in the configuration settings above to activate polling.
        </div>
      )}
      ...
      <div className="mt-6 rounded-md border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">
        ...
        <p className="mb-2">
          This example demonstrates the recommended patterns for using the siteConfiguration
          feature:
        </p>
        <ul className="ml-5 list-disc space-y-1">
          <li>Using typed selectors to access configuration values (see Step 1)</li>
          <li>
            Setting up useEffect with proper dependencies to respond to configuration changes (Step
            4)
          </li>
