  ...
  return (
    <Form onSubmit={handleSubmit}>
      ...
      <FormFieldWithHelp
        ...
        htmlFor="dataType"
        required
        helpText="Select the data type based on device documentation. For multi-register values like 32-bit integers or floats, word count is automatically adjusted."
      >
        <Select
          id="dataType"
      ...
      <Form.Row>
        <FormFieldWithHelp
          ...
          helpText={
            isMultiRegister
              ? 'Specifies the byte order for multi-register values. ABCD is big-endian (most significant byte first), DCBA is little-endian.'
              : 'Specifies the byte order. AB is big-endian (most significant byte first), BA is little-endian.'
          }
        >
          <Select
            id="byteOrder"
      ...
      <Form.Row>
        ...
        <FormFieldWithHelp
          ...
          required
          helpText={`Starting index within the selected register range. For multi-register values, this is the index of the first register.`}
        >
          <Input
            id="registerIndex"
            name="registerIndex"
      ...
      {isBitType && (
        <FormFieldWithHelp
          ...
          htmlFor="bitPosition"
          required
          helpText="For bit/boolean types, specifies which bit to extract (0-15). Bit 0 is least significant."
        >
          <Input
            id="bitPosition"
            name="bitPosition"
        ...
        <>
          <Form.Row>
            <FormFieldWithHelp
              ...
              helpText="Simple multiplier for the raw value. For example, use 0.1 to divide by 10."
            >
              <Input
                id="scalingFactor"
                name="scalingFactor"
            ...
            {isNumericType && (
              <FormFieldWithHelp
                ...
                helpText="JavaScript expression using 'x' as the variable. For example: 'x * 0.1 + 32' to convert Celsius to Fahrenheit."
              >
                <Input
                  id="scalingEquation"
          ...
          <Form.Row>
            <FormFieldWithHelp
              ...
              helpText="Number of decimal places to display in the UI."
            >
              <Input
                id="decimalPoint"
                name="decimalPoint"
            ...
            <FormFieldWithHelp
              label="Unit"
              htmlFor="unit"
              helpText="Unit of measurement, such as °C, bar, etc."
            >
              <Input
                id="unit"
                name="unit"
          ...
          {!isBitType && (
            <FormFieldWithHelp
              label="Bitmask"
              htmlFor="bitmask"
              helpText="Hexadecimal mask to extract specific bits (e.g., 0xFF00 for high byte). Leave empty for no masking."
            >
              <Input
                id="bitmask"
                name="bitmask"
          ...
          {isNumericType && (
            <Form.Row>
              <FormFieldWithHelp
                label="Minimum Value"
                htmlFor="minValue"
                helpText="Optional minimum valid value. Values below this will be marked as errors."
              >
                <Input
                  id="minValue"
                  name="minValue"
              ...
              <FormFieldWithHelp
                label="Maximum Value"
                htmlFor="maxValue"
                helpText="Optional maximum valid value. Values above this will be marked as errors."
              >
                <Input
                  id="maxValue"
                  name="maxValue"
          ...
          <FormFieldWithHelp
            label="Description"
            htmlFor="description"
            helpText="Optional description of this parameter for documentation purposes."
          >
            <Input
              id="description"
              name="description"
      ...
      <Form.Actions>
        ...
        <Button type="submit">{initialData ? 'Update Parameter' : 'Add Parameter'}</Button>
      </Form.Actions>
    </Form>
  );
};
