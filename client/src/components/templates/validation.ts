// client/src/components/devices/NewTemplateForm/validation.ts
import { TemplateFormState } from './TemplateFormContext';
...
export const validateRegisterRanges = (
  registerRanges: TemplateFormState['registerRanges']
): Record<string, string> => {
  const errors: Record<string, string> = {};
...
const checkParameterOverlaps = (
  parameters: TemplateFormState['parameters']
): Record<string, string> => {
  const errors: Record<string, string> = {};
  ...
  const paramsByRange: Record<
    string,
    Array<{ index: number; param: TemplateFormState['parameters'][0]; end: number }>
  > = {};
...
): TemplateFormState['validationState'] => {
  const validationState: TemplateFormState['validationState'] = {
