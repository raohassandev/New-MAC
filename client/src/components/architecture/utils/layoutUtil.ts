...
export function generateSystemLayout(
  ...
  // Default config
  const config = layoutConfigs[algorithm as keyof typeof layoutConfigs] || layoutConfigs.hierarchical;
  ...
  const newNodes = nodes.map((node) => {
    ...
    let position = { x: dagreNode.x - config.nodeWidth / 2, y: dagreNode.y - config.nodeHeight / 2 };
...
function getRadialPosition(
  ...
  const nodeAngle = typeBaseAngle + (typeIndex / Math.max(1, typeTotalCount - 1)) * (typeAngleSpan * 0.8);
...
export function layerBasedLayout(nodes: SystemNode[], edges: SystemEdge[]): { newNodes: SystemNode[]; newEdges: SystemEdge[] } {
  // Group nodes by type
