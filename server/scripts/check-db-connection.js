...
const testDatabaseConnection = async (dbInfo) => {
  ...
  try {
    ...
    connection = await mongoose.createConnection(dbInfo.uri, {
    ...
    for (const expectedCollection of dbInfo.collections) {
      if (collectionNames.includes(expectedCollection)) {
        console.log(`  ${colors.green}✓ Expected collection found:${colors.reset} ${expectedCollection}`);
        ...
        console.log(`  ${colors.yellow}⚠ Expected collection missing:${colors.reset} ${expectedCollection}`);
      }
    }
    
    return true;
  } catch (error) {
    console.log(`${colors.red}✗ Failed to connect to ${dbInfo.name}:${colors.reset} ${error.message}`);
    return false;
  } finally {
...
const testDatabaseConnections = async () => {
  ...
  console.log(`${colors.magenta}Testing MACSYS database connections ${new Date().toISOString()}${colors.reset}\n`);
  ...
  console.log(`  LIBRARY_DB_URI: ${process.env.LIBRARY_DB_URI || 'not set (using default localhost)'}`);
  ...
  if (successCount === databases.length) {
    console.log(`${colors.green}All database connections successful (${successCount}/${databases.length})${colors.reset}`);
  } else {
    console.log(`${colors.yellow}Some database connections failed (${successCount}/${databases.length} successful)${colors.reset}`);
