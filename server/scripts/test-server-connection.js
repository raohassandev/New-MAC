...
const testEndpoint = (port, endpoint) => {
  return new Promise((resolve, reject) => {
    console.log(`${colors.cyan}Testing endpoint:${colors.reset} http://localhost:${port}${endpoint}`);
...
const testServer = async () => {
  ...
  console.log(`${colors.magenta}Testing MACSYS server connection ${new Date().toISOString()}${colors.reset}\n`);
  ...
  for (const endpoint of TEST_ENDPOINTS) {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        console.log(`${colors.yellow}⚠ ${endpoint} - Status:${colors.reset} ${response.statusCode}`);
        console.log(`  Response: ${JSON.stringify(response.body, null, 2).substring(0, 150)}...\n`);
        hasErrors = true;
      }
    } catch (error) {
  ...
  if (hasErrors) {
    console.log(`${colors.yellow}Some tests failed. The server may not be running correctly on port ${port}.${colors.reset}`);
    console.log(`${colors.yellow}Try starting the server with: npm start${colors.reset}`);
  } else {
    console.log(`${colors.green}All tests passed! Server is running correctly on port ${port}.${colors.reset}`);
  }
