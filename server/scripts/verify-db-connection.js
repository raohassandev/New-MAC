...
async function verifyDatabaseConnection() {
  try {
    ...
    if (deviceCollection) {
      ...
      const count = await mongoose.connection.db.collection(deviceCollection.name).countDocuments();
      console.log(`Total devices in collection: ${count}`);
      
      // Check sample device
      if (count > 0) {
        const sampleDevice = await mongoose.connection.db.collection(deviceCollection.name).findOne();
        console.log('\nSample device document structure:');
        console.log(JSON.stringify({
          _id: sampleDevice._id,
          name: sampleDevice.name,
          connectionType: sampleDevice.connectionType || 
                         (sampleDevice.connectionSetting ? sampleDevice.connectionSetting.connectionType : 'N/A'),
          hasDataPoints: sampleDevice.dataPoints ? `Yes (${sampleDevice.dataPoints.length})` : 'No',
          hasRegisters: sampleDevice.registers ? `Yes (${sampleDevice.registers.length})` : 'No'
        }, null, 2));
      }
    } else {
    ...
    try {
      ...
      const TestDevice = mongoose.model(`TestDevice_${timestamp}`, DeviceSchema);
