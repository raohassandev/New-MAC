...
async function verifyDatabases() {
  ...
  try {
    ...
    if (!process.env.MONGO_URI) {
      console.log('WARNING: MONGO_URI not found in environment. Using default: mongodb://localhost:27017/client');
    }
    
    if (!process.env.LIBRARY_DB_URI) {
      console.log('WARNING: LIBRARY_DB_URI not found in environment. Using default: mongodb://localhost:27017/amx');
    }
    ...
    clientOnlyCollections.forEach(collection => {
      if (!clientCollections.some(c => c.name === collection) && collection !== 'alerts') {
        ...
        if (collection === 'alerts') {
          console.log(`NOTE: ${collection} collection not found in client DB - may be created later`);
        } else {
