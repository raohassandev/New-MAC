...
const getDeviceDriverModel = (): mongoose.Model<any> => {
  if (mongoose.connection.db && mongoose.connection.db.databaseName !== 'amx') {
    console.error(
      'WARNING: Using wrong database connection for DeviceDriver! Should be using AMX database but currently using:',
  ...
  try {
    ...
    if (app && app.locals && app.locals.libraryModels) {
      ...
      console.error(
        'App locals or libraryModels not found for DeviceDriver, falling back to dedicated connection',
      );
    }
  } catch (error) {
  ...
  try {
    ...
    let amxConn = mongoose.connections.find(conn => conn.db && conn.db.databaseName === 'amx');
    ...
    console.warn(
      'Falling back to default mongoose connection for DeviceDriver - this may use the wrong database!',
    );
...
const getDeviceTypeModel = (): mongoose.Model<any> => {
  if (mongoose.connection.db && mongoose.connection.db.databaseName !== 'amx') {
    console.error(
      'WARNING: Using wrong database connection! Should be using AMX database but currently using:',
      mongoose.connection.db.databaseName,
    );
  }
...
export const createDeviceDriver = async (req: Request, res: Response) => {
  try {
    ...
    if (!['admin', 'devicedriver_manager'].includes(userReq.user.role)) {
      return res
        .status(403)
        .json({ message: 'You do not have permission to create device drivers' });
    }
...
export const updateDeviceDriver = async (req: Request, res: Response) => {
  try {
    ...
    if (!['admin', 'devicedriver_manager'].includes(userReq.user.role)) {
      return res
        .status(403)
        .json({ message: 'You do not have permission to update device drivers' });
    }
    ...
    if (deviceDriver.createdBy?.userId !== userReq.user.id && userReq.user.role !== 'admin') {
      return res
        .status(403)
        .json({ message: 'You do not have permission to update this device driver' });
    }

    const updatedDeviceDriver = await DeviceDriver.findByIdAndUpdate(id, req.body, {
      new: true,
      runValidators: true,
    });
...
export const deleteDeviceDriver = async (req: Request, res: Response) => {
  try {
    ...
    if (userReq.user.role !== 'admin') {
      return res
        .status(403)
        .json({ message: 'You do not have permission to delete device drivers' });
    }
    ...
    if (deviceDriver.createdBy?.userId !== userReq.user.id && userReq.user.role !== 'admin') {
      return res
        .status(403)
        .json({ message: 'You do not have permission to delete this device driver' });
    }
...
export const createDeviceType = async (req: Request, res: Response) => {
  try {
    ...
    if (!userReq.user) {
      ...
      return res
        .status(401)
        .json({ message: 'Authentication required - no user found in request' });
    }
    ...
    res
      .status(500)
      .json({ message: 'Failed to create device type: ' + (error.message || 'Unknown error') });
  }
};
...
export const updateDeviceType = async (req: Request, res: Response) => {
  try {
    ...
    if (deviceType.createdBy?.userId !== userReq.user.id && userReq.user.role !== 'admin') {
      return res
        .status(403)
        .json({ message: 'You do not have permission to update this device type' });
    }

    const updatedDeviceType = await DeviceType.findByIdAndUpdate(id, req.body, {
      new: true,
      runValidators: true,
    });
