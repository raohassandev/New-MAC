...
export function verifyRoutes(app: Express): void {
  ...
  try {
    ...
    function findRoute(method: string, path: string): boolean {
      ...
      function checkLayer(layer: any, basePath: string = '') {
        if (layer.route) {
          ...
          if (
            routeMethods.includes(method.toUpperCase()) &&
            (fullPath === path ||
              (path.includes(':') &&
                new RegExp('^' + fullPath.replace(/:[^/]+/g, '[^/]+') + '$').test(path)))
          ) {
            found = true;
    ...
    if (allFound) {
      ...
      console.log(
        '[Route Verification] ⚠️ Some critical API routes are missing. Check route configuration.',
      );
    }
  } catch (error) {
