...
export class PollingService extends EventEmitter {
  ...
  public configureDevice(deviceId: string, interval: number, parameters?: string[]): boolean {
    ...
    this._logger.info('PollingService', `Configured polling for device ${deviceId} with interval ${interval}ms`);
  ...
  public start(deviceId: string): boolean {
    ...
    this._logger.info('PollingService', `Starting polling for device ${deviceId} with interval ${config.interval}ms`);
    ...
    for (const [deviceId, config] of this._pollingConfigs.entries()) {
      if (config.enabled) {
        try {
          ...
          this._logger.error('PollingService', `Error starting polling for device ${deviceId}:`, error);
  ...
  public stopAll(): Map<string, boolean> {
    ...
    for (const deviceId of this._pollingIntervals.keys()) {
      try {
        ...
        this._logger.error('PollingService', `Error stopping polling for device ${deviceId}:`, error);
        results.set(deviceId, false);
      }
    }
  ...
  private async _pollDevice(deviceId: string): Promise<ParameterValue[]> {
    ...
    if (config?.parameters) {
      ...
      for (const paramName of config.parameters) {
        ...
        if (param) {
          ...
          this._logger.warn('PollingService', `Parameter ${paramName} not found for device ${deviceId}`);
        }
      }
    } else {
      // Use all parameters
    ...
    if (this._debugMode) {
      this._logger.debug('PollingService', `Polling ${parameters.length} parameters for device ${deviceId}`);
    }
    ...
    try {
      ...
      if (this._debugMode) {
        this._logger.debug('PollingService', `Polled ${result.data.length} parameters for device ${deviceId}`);
      }
