...
import { Device, Protocol } from './device.interface';
import {
  Parameter,
  ParameterValue,
  ConnectionState,
...
export class DeviceBase extends EventEmitter implements Device {
  ...
  /**
   * Read multiple registers directly
   * @param registerType Register type
   * @param address Start address
   * @param count Number of registers to read
   * @returns Promise resolving to array of register values
   */
  async readMultipleRegisters(
    ...
    // Method needed by the adapter for backward compatibility
    // Implementation depends on the protocol used
...
/**
 * Specialized device implementation for Modbus
 */
export class ModbusDevice extends DeviceBase {
  /**
  ...
  constructor(options: {
    ...
    super(options.id, options.name, options.client, options.parameters || [], options.description);
  }
  ...
  async readMultipleRegisters(
    ...
    try {
      // Create a temporary parameter for this read operation
      const tempParam: Parameter = {
        id: `temp_read_${registerType}_${address}_${count}`,
        name: `temp_read_${registerType}_${address}_${count}`,
        address,
        registerType,
      ...
      // Use the protocol to read the registers
      const result = await this.protocol.readParameter(tempParam);

      if (!result.success || !result.data) {
        throw new Error(`Failed to read registers: ${result.error?.message || 'Unknown error'}`);
      }

      // Return the data (should be an array of register values)
      return Array.isArray(result.data) ? result.data : [result.data];
    } catch (error) {
      throw new Error(`Error reading registers at ${address}: ${
