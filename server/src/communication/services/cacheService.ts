...
export class CacheService extends EventEmitter {
   ...
   * @param ttl Time-to-live in milliseconds (optional)
   */
  public setValues(deviceId: string, values: ParameterValue[], ttl?: number): void {
   ...
   * @param includeExpired Whether to include expired values
   */
  public getAllValues(deviceId: string, includeExpired: boolean = false): ParameterValue[] {
    const deviceCache = this.cache.get(deviceId);
    if (!deviceCache) {
      return [];
    }
   ...
   * @param includeExpired Whether to include expired values
   */
  public hasValue(deviceId: string, paramName: string, includeExpired: boolean = false): boolean {
    const deviceCache = this.cache.get(deviceId);
    if (!deviceCache) {
      return false;
    }
