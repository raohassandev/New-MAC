...
export class DeviceManager extends EventEmitter {
    ...
    public registerDevice(device: Device): void {
        ...
        device.on('stateChanged', ((event: Event) => {
            ...
            this.emit(createEvent('deviceStateChanged', 'DeviceManager', {
                deviceId: device.id,
                state
            }));
        }) as EventListener);
        ...
        device.on('error', ((event: Event) => {
            ...
            this.emit(createEvent('deviceError', 'DeviceManager', {
                deviceId: device.id,
                error
            }));
        }) as EventListener);
        
        this.emit(createEvent('deviceRegistered', 'DeviceManager', {
            deviceId: device.id
        }));
    }
    ...
    public unregisterDevice(deviceId: string): void {
        ...
        this.emit(createEvent('deviceUnregistered', 'DeviceManager', {
            deviceId
        }));
    }
    ...
    public registerProtocol(protocolId: string, protocol: Protocol): void {
        ...
        protocol.on('stateChanged', ((event: Event) => {
            const state = event.data?.state as ConnectionState;
            this.emit(createEvent('protocolStateChanged', 'DeviceManager', {
                protocolId,
                state
            }));
        }) as EventListener);
        ...
        protocol.on('error', ((event: Event) => {
            const error = event.data?.error as Error;
            this.emit(createEvent('protocolError', 'DeviceManager', {
                protocolId,
                error
            }));
        }) as EventListener);
        
        this.emit(createEvent('protocolRegistered', 'DeviceManager', {
            protocolId
        }));
    }
    ...
    public unregisterProtocol(protocolId: string): void {
        ...
        this.emit(createEvent('protocolUnregistered', 'DeviceManager', {
            protocolId
        }));
    }
