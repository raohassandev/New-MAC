...
import { ConnectionError, TimeoutError, createErrorFromException } from '../../../../core/errors';
import { ConnectionState, ConnectionOptions, RtuConnectionOptions } from '../../../../core/types';
...
export class ModbusRtuConnection extends EventEmitter {
  ...
  public async connect(): Promise<void> {
    ...
    try {
      ...
      this.parser = this.port.pipe(
        new SerialPort.parsers.Delimiter({ delimiter: Buffer.from([]) }),
      );
  ...
  public async sendRequest(address: number, functionCode: number, data: Buffer): Promise<Buffer> {
    ...
    const adu = Buffer.concat([Buffer.from([address]), pdu]);
  ...
  private validateResponse(response: Buffer): void {
    ...
    if (this.currentRequest && response[0] !== this.currentRequest.address) {
      throw new Error(
        `Unexpected slave address: ${response[0]}, expected: ${this.currentRequest.address}`,
      );
    }
    ...
    if (this.currentRequest) {
      ...
      if ((functionCode & 0x80) !== 0) {
        if ((functionCode & 0x7f) === expectedFunctionCode) {
          ...
          throw new Error(
            `Modbus exception: function code ${expectedFunctionCode}, exception code ${exceptionCode}`,
          );
        } else {
          throw new Error(
            `Unexpected exception function code: ${functionCode}, expected: ${expectedFunctionCode}`,
          );
        }
      }
      ...
      if (functionCode !== expectedFunctionCode) {
        throw new Error(
          `Unexpected function code: ${functionCode}, expected: ${expectedFunctionCode}`,
        );
      }
    }
  }
  ...
  private processQueue(): void {
    if (this.requestQueue.length > 0 && !this.currentRequest) {
      ...
      const pdu = Buffer.concat([
        Buffer.from([this.currentRequest.functionCode]),
        this.currentRequest.data,
      ]);
      const adu = Buffer.concat([Buffer.from([this.currentRequest.address]), pdu]);
