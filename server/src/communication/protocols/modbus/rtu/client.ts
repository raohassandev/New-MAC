...
import { RTUConnectionSettings, ModbusClient } from '../../../core/types';
...
export class ModbusRTUClient implements ModbusClient {
  ...
  private options: RTUConnectionSettings;
   ...
   * @param options RTU connection settings
   */
  constructor(options: RTUConnectionSettings) {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      logService.debug(`Connected to Modbus RTU device at ${this.options.path}`);
    } catch (error) {
      this.connected = false;
      this.connectInProgress = false;
      
      logService.error(`Failed to connect to Modbus RTU device: ${error}`);
      throw error;
    }
  }
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      ...
      logService.debug(`Disconnected from Modbus RTU device at ${this.options.path}`);
    } catch (error) {
      logService.error(`Failed to disconnect from Modbus RTU device: ${error}`);
      throw error;
    }
  }
