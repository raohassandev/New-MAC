...
export class ModbusPDU {
  ...
  static parseResponse(data: Buffer, functionCode: ModbusFunctionCode): Buffer {
    ...
    if (responseCode !== functionCode) {
      throw new ModbusError(
        `Unexpected function code in response: expected ${functionCode}, got ${responseCode}`,
      );
    }
  ...
  static parseReadCoilsResponse(data: Buffer, quantity: number): boolean[] {
    ...
    if (responsePdu.length !== byteCount + 1) {
      throw new Error(
        `Invalid read coils response: expected ${byteCount + 1} bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseReadDiscreteInputsResponse(data: Buffer, quantity: number): boolean[] {
    ...
    if (responsePdu.length !== byteCount + 1) {
      throw new Error(
        `Invalid read discrete inputs response: expected ${byteCount + 1} bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseReadHoldingRegistersResponse(data: Buffer): number[] {
    ...
    if (responsePdu.length !== byteCount + 1) {
      throw new Error(
        `Invalid read holding registers response: expected ${byteCount + 1} bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseReadInputRegistersResponse(data: Buffer): number[] {
    ...
    if (responsePdu.length !== byteCount + 1) {
      throw new Error(
        `Invalid read input registers response: expected ${byteCount + 1} bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseWriteSingleCoilResponse(data: Buffer): { address: number; value: boolean } {
    ...
    if (responsePdu.length !== 4) {
      throw new Error(
        `Invalid write single coil response: expected 4 bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseWriteSingleRegisterResponse(data: Buffer): { address: number; value: number } {
    ...
    if (responsePdu.length !== 4) {
      throw new Error(
        `Invalid write single register response: expected 4 bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseWriteMultipleCoilsResponse(data: Buffer): { startAddress: number; quantity: number } {
    ...
    if (responsePdu.length !== 4) {
      throw new Error(
        `Invalid write multiple coils response: expected 4 bytes, got ${responsePdu.length}`,
      );
    }
   ...
   * @param data The response PDU
   */
  static parseWriteMultipleRegistersResponse(data: Buffer): {
    startAddress: number;
    quantity: number;
  } {
    // Validate the response
    ...
    if (responsePdu.length !== 4) {
      throw new Error(
        `Invalid write multiple registers response: expected 4 bytes, got ${responsePdu.length}`,
      );
    }
   ...
   * @param data The response PDU
   */
  static parseMaskWriteRegisterResponse(data: Buffer): {
    address: number;
    andMask: number;
    orMask: number;
  } {
    ...
    if (responsePdu.length !== 6) {
      throw new Error(
        `Invalid mask write register response: expected 6 bytes, got ${responsePdu.length}`,
      );
    }
  ...
  static parseReadWriteMultipleRegistersResponse(data: Buffer): number[] {
    ...
    if (responsePdu.length !== byteCount + 1) {
      throw new Error(
        `Invalid read/write multiple registers response: expected ${byteCount + 1} bytes, got ${responsePdu.length}`,
      );
    }
