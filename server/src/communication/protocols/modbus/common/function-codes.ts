...
export function createModbusError(exceptionCode: ModbusExceptionCode): ModbusError {
  ...
  switch (exceptionCode) {
    case ModbusExceptionCode.ILLEGAL_FUNCTION:
      message = 'Illegal Function: The function code received in the query is not recognized or allowed by the server';
      break;
    case ModbusExceptionCode.ILLEGAL_DATA_ADDRESS:
      message = 'Illegal Data Address: The data address received in the query is not an allowable address for the server';
      break;
    case ModbusExceptionCode.ILLEGAL_DATA_VALUE:
      message = 'Illegal Data Value: A value contained in the query data field is not an allowable value for the server';
      break;
    case ModbusExceptionCode.SERVER_DEVICE_FAILURE:
      message = 'Server Device Failure: An unrecoverable error occurred while the server was attempting to perform the requested action';
      break;
    case ModbusExceptionCode.ACKNOWLEDGE:
      message = 'Acknowledge: The server has accepted the request but will require a long time to process it';
      break;
    ...
    case ModbusExceptionCode.NEGATIVE_ACKNOWLEDGE:
      message = 'Negative Acknowledge: The server cannot perform the program function received in the query';
      break;
    ...
    case ModbusExceptionCode.GATEWAY_PATH_UNAVAILABLE:
      message = 'Gateway Path Unavailable: The gateway was unable to allocate an internal communication path';
      break;
