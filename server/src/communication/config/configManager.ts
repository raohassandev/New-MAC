...
export class ConfigManager extends EventEmitter {
  ...
  public removeParameter(deviceId: string, parameterName: string): void {
    ...
    this.logger.info(
      `Parameter removed: ${parameterName} from device ${deviceId}`,
      'ConfigManager',
    );
  }
    ...
    if (device.pollGroups) {
      ...
      for (const pollGroup of device.pollGroups) {
        ...
        if (!Array.isArray(pollGroup.parameterNames)) {
          throw new Error(
            `Poll group ${pollGroup.id} in device ${device.id} must have a parameterNames array`,
          );
        }
        ...
        if (
          pollGroup.intervalMs !== undefined &&
          (typeof pollGroup.intervalMs !== 'number' || pollGroup.intervalMs <= 0)
        ) {
          throw new Error(
  ...
  private validateParameterConfig(parameter: ParameterConfig): void {
    ...
    if (
      (parameter.dataType === 'string' || parameter.dataType === 'byteArray') &&
      (!parameter.length || parameter.length <= 0)
    ) {
      throw new Error(
        `Parameter ${parameter.name} of type ${parameter.dataType} must have a positive length`,
      );
    }

    // Validate bit index
    if (parameter.bitIndex !== undefined && (parameter.bitIndex < 0 || parameter.bitIndex > 15)) {
      throw new Error(
        `Parameter ${parameter.name} has invalid bit index: ${parameter.bitIndex}. Must be between 0 and 15.`,
      );
    }
    ...
    if (parameter.scaling !== undefined && typeof parameter.scaling !== 'number') {
      throw new Error(
        `Parameter ${parameter.name} has invalid scaling factor: ${parameter.scaling}. Must be a number.`,
      );
    }

    // Validate alarm and warning thresholds
    if (
      parameter.alarmHigh !== undefined &&
      parameter.alarmLow !== undefined &&
    ...
    if (
      parameter.warningHigh !== undefined &&
      parameter.warningLow !== undefined &&
    ...
    if (parameter.deadband !== undefined && parameter.deadband < 0) {
      throw new Error(
        `Parameter ${parameter.name} has invalid deadband: ${parameter.deadband}. Must be non-negative.`,
      );
    }
  }
}
