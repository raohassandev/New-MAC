...
export class ConfigLoader {
  ...
  public async loadAndApply(configPath?: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to load and apply configuration: ${formattedError.message}`,
        'ConfigLoader',
      );
  ...
  private async createAndRegisterDevices(deviceConfigs: DeviceConfig[]): Promise<void> {
    for (const deviceConfig of deviceConfigs) {
      try {
        ...
        // Register protocol
        this.deviceManager.registerProtocol(`${deviceConfig.id}_protocol`, protocol);
        ...
        this.logger.info(
          `Device ${deviceConfig.id} created and registered successfully`,
          'ConfigLoader',
        );
      } catch (error) {
        const formattedError = createErrorFromException(error);
