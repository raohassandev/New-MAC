...
async function checkDatabase() {
  ...
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      console.log(
        'Default mongoose connection is NOT ACTIVE (state:',
    ...
    try {
      ...
      if (models.Device) {
        ...
        if (deviceCount > 0) {
          ...
          console.log('\nSample device:');
          console.log(`- ID: ${sampleDevice ? sampleDevice._id : 'N/A'}`);
          console.log(`- Name: ${sampleDevice ? sampleDevice.name : 'N/A'}`);
          console.log(`- Type: ${sampleDevice ? (sampleDevice.make || 'N/A') : 'N/A'} / ${sampleDevice ? (sampleDevice.model || 'N/A') : 'N/A'}`);
        }
      }
      ...
      const rawDevices = await clientConnection.db
        .collection('devices')
        .find({})
        .limit(5)
        .toArray();
