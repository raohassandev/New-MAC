...
export const ensureClientDeviceModel = async (
  ...
  const isValidModel =
    DeviceModel && DeviceModel.db?.readyState === 1 && DeviceModel.db?.name === 'client';
  ...
  console.log(
    `[dbHelper] ${forceReconnect ? 'Forcing reconnect' : 'No valid model'}, creating new connection`,
  );
  ...
  if (req?.app?.locals) {
    if (req.app.locals.clientModels?.Device) {
      ...
      if (DeviceModel && DeviceModel.db?.name === 'client' && DeviceModel.db?.readyState === 1) {
    ...
    if (req.app.locals.mainDB) {
      ...
      if (mainDB.readyState === 1 && mainDB.name === 'client') {
        ...
        console.warn(
          `[dbHelper] mainDB is available but not connected to client database (state: ${mainDB.readyState}, name: ${mainDB.name})`,
        );
      }
    }
  }
